...
> Tracker en vivo de nuevas implementaciones, herramientas y patrones RAG — actualizado cada 15 minutos
...
RAG Radar monitorea continuamente GitHub en busca de proyectos, librerías y patrones de producción de generación aumentada por recuperación (RAG) recién publicados. Puntúa y categoriza los descubrimientos por tipo de componente (chunking, embedding, recuperación, generación) y genera un feed estructurado al que los desarrolladores pueden suscribirse o explorar. Funciona con GitHub Actions y la GitHub Search API.

Esta lista se **actualiza automáticamente cada 15 minutos** mediante un cron de GitHub Actions. Cada commit refleja un cambio real en la fuente de datos upstream — nuevos ítems añadidos, ítems expirados eliminados — así que puedes confiar en que lo que ves está al día.
